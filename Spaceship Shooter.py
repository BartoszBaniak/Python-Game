...
import random
import math
from settings import screenWidth, screenHeight, boss_image, boss_death, boss_attack, boss_engine, boss1_image, boss1_death, boss1_attack, boss1_engine, enemy_ship_image, enemy_death, enemy_attack, enemy_engine, enemy2_ship_image, enemy2_death, enemy2_attack, enemy2_engine, enemy1_ship_image, enemy1_death, enemy1_attack, enemy1_engine, level, boss_level, font, font2, font3, font4, font5
...
class boss(object):
    def __init__(self, x, y, width, height, health):
        if level > 10:
            self.fly = pygame.transform.scale(boss_image, (384, 384))
            self.engine = boss_engine
            self.death = boss_death
        else:
            self.fly = pygame.transform.scale(boss1_image, (384, 384))
            self.engine = boss1_engine
            self.death = boss1_death
        self.x = x
        self.y = y
        self.width = width
        self.height = height
    ...
    def draw(self, win):
        if self.is_alive:
            ...
            if self.engine_frame >= len(self.engine):
                self.engine_frame = 0
            ...
            if self.engine_frame < len(self.engine):
                win.blit(self.engine[self.engine_frame], (self.x, self.y))
        ...
        if self.is_alive == False and self.health <= 5:
            ...
            if self.death_frame < len(self.death):
                win.blit(self.death[self.death_frame], (self.x, self.y))

            if self.health == 0 and self.death_frame >= len(self.death):
                self.removed = True

    def death_animation_finished(self):
        return self.is_alive == False and self.death_frame >= len(self.death)
    ...
    def shoot(self):
        if level > 10:
            ...
            new_bullet = boss_projectile(self.x + 45 + self.width//2, self.y + 35 + self.height)
...
class boss_projectile(object):
    def __init__(self, x, y):
        if level > 10:
            ...
            self.y = y + 100
            self.vel = 15
            self.attack = boss1_attack
            self.hitbox = (self.x + 35, self.y + 60, 120, 60)
    ...
    def draw(self, win):
        ...
        if self.projectile_frame >= len(self.attack):
            self.projectile_frame = 0
        ...
        if self.projectile_frame < len(self.attack):
            win.blit(self.attack[self.projectile_frame], (self.x, self.y))

# PRZECIWNIK
class enemy(object):    
    def __init__(self, x, y, width, height, health):
        if level > 10:
            self.fly = pygame.transform.scale(enemy2_ship_image, (128, 128))
            self.engine = enemy2_engine
            self.death = enemy2_death
        elif level > 5:
            self.fly = pygame.transform.scale(enemy1_ship_image, (128, 128))
            self.engine = enemy1_engine
            self.death = enemy1_death
        else:
            self.fly = pygame.transform.scale(enemy_ship_image, (128, 128))
            self.engine = enemy_engine
            self.death = enemy_death
        self.x = x
        self.y = y
        self.width = width
        self.height = height
...
class enemy_projectile(object):
    def __init__(self, x, y):
        if level > 10:
            self.attack = enemy2_attack
        elif level > 5:
            self.attack = enemy1_attack
        else:
...
def update_difficulty(level):
    health_increase = level * 10
    return health_increase

#LEVEL CREATOR
def create_inf_levels(level):
    enemy_count = min(2 + round(level/4), 4)
    enemy_count1 = enemy_count + 1
    health_increase = update_difficulty(level)
    enemies = []
    if level % boss_level == 0:
        x = 860
        y = 300
        boss_enemy = boss(x, y, 64, 64, level * 100)
        enemies.append(boss_enemy)
    else:
        for i in range(enemy_count):
            for j in range(enemy_count1):
...
while run:
    ...
    if game_state == in_menu:
        ...
        if shootLoop > 25 and score <= 200:
            shootLoop = 0
        if shootLoop > 15 and score > 200 and score <= 5000:
            shootLoop = 0
        if shootLoop > 7 and score > 5000:
            shootLoop = 0
        ...
        if all_enemies_dead:
            if level == 0:
                ...
                ship.x = 860
                ship.y = 860
                enemies = create_inf_levels(level)
            else: 
                level_transition()
                level += 1
                ship.x = 860
        ...
        if ship.is_alive:
            if keys[pygame.K_SPACE] and shootLoop == 0:
                ...
                elif score > 3000 and score <= 7000:
                    bullets.append(Projectile(ship.x + 40, ship.y + ship.height//2)) #lewo
                    bullets.append(Projectile(ship.x + 80, ship.y + ship.height//2)) #srodek
                    bullets.append(Projectile(ship.x + 120, ship.y + ship.height//2)) #prawo
